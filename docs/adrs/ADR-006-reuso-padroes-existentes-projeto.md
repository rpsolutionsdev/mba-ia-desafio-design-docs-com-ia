...
* **Origem no Código:** `src/shared/errors/app-error.ts`, `src/middlewares/error.middleware.ts`, `src/middlewares/auth.middleware.ts`, `src/shared/logger/index.ts`, `src/modules/orders/order.service.ts`
...
* Logging estruturado em formato JSON padronizado com a biblioteca Pino (`src/shared/logger/index.ts`).
...
4. **Logging com Pino:** Instrumentar todos os pontos do ciclo de vida dos webhooks (inserção no outbox, tentativas do worker, assinaturas HMAC e replays) com o logger centralizado `src/shared/logger/index.ts`.
